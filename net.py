...
def deconv2d(x, W, output_shape, stride, padding='SAME'):
    return tf.nn.conv2d_transpose(x, W, output_shape, [1, stride, stride, 1], padding=padding)
...
def crop_and_concat3_2d(x1, x2, x3,):
...
def batchnorm2d(x, is_training, eps=1e-8, decay=0.9, name='BatchNorm2d'):
...
def M_MSMHV_CNN(xs, is_training=True):
